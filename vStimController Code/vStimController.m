...
% Set screen calibration values (y = a*x^b, where y luminance cd/m^2 and x
...
try
    ...
    % Set monitor calibration values
    a = handles.USER.a;     
    b = handles.USER.b;
   
    % Set white, black, gray luminance values
    white = a*255^b;
    black = 0;
    gray = 0.5*(black + white);
    
    % Set white, black, gray pixel values
    whitePixVal = lum2pixVal(white);
    blackPixVal = lum2pixVal(black);
    grayPixVal = lum2pixVal(gray);
    
    % Open gray screen
    HideCursor
    [w,wRect] = Screen(screenNum,'OpenWindow',grayPixVal);
    Screen('DrawText',w,sprintf('Generating stimuli'),10,30,whitePixVal);
    Screen('Flip',w);
    ...
    % Add blank stimulus. Last stim condition is set to NaN for all
    % parameters
    if p.addBlank
        temp = fieldnames(stim);
        for i = 1:length(temp)
            if strcmp(temp{i},'code')
                stim.(temp{i})(end+1) = max(stim.(temp{i})) + 1;
            else
                stim.(temp{i})(end+1) = NaN;
            end
        end
    end
    
    % Put stim struct in base
        ...
        switch stimType
            ...
            case 'Drifting gratings'
                for s = stimInd
                    ...
                    fwrite(u,[fname '*' stimType '*' num2str(s)]);
                    
%                     [fname '*' stimType '*' num2str(s)]
                    ...
                    if p.addBlank && s == max(stimInd)
                        while(vbl < vblEndTime)
                            
                            i = i + 1;
                            Screen('FillRect',w,grayPixVal);
...
for i = 1:length(fields)
    switch fields{i}
        ...
        case {'randomize','interleave','addBlank'}
...
d.ScreenSizecmX = 43.18;          % Put in RigSpecific
d.ScreenSizecmY = 27.3;        % Put in RigSpecific
...
% --- Executes on button press in addBlank.
function addBlank_Callback(hObject, eventdata, handles)
% hObject    handle to addBlank (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of addBlank
