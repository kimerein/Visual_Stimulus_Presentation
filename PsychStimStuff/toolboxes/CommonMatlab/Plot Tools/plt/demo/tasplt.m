...
traceID = reshape(sprintf('%2d,000 ft',alt),9,length(alt))';
traceID(1,:) = 'sea level';               % change '0,000 ft' to 'sea level'
...
plt(gph,mpg,...
    'Position',[10 45 wid hi],'Options','S-X-Y',...
    ...
    'AxisCB',['set(findobj(gcf,"tag","tas1"),' ys yt xs],...
...
axr = axes('tag','tas1','XaxisLoc','top','YaxisLoc','right','pos',get(ax1,'pos'),...
...
plt(gph,tas,'Position',[sz(3)-wid-10 sz(4)-hi-80 wid hi],'Options','S-X-Y',...
    ...
    'Xstring','sprintf("%3.1f%% power",5*@XVAL)',...
    'AxisCB',['set(findobj(gcf,"tag","tas2"),' xs],...
    'LabelX','Fuel flow (gph)','AxisPos',[1 1 1 .95],...
...
% Display 'Percent Power' as an alternative x-axis units above the graph
ax2 = gca;
axr = axes('tag','tas2','XaxisLoc','top','pos',get(ax2,'pos'),...
           'xcolor',ac,'ycolor',get(gcf,'color'));
set(get(axr,'xlabel'),'string','Percent Power');
axes(ax2); % put primary axis on top

% close the 1st plot when the 2nd plot is closed and visa versa
set([gcf hfig],'tag','tas','closeReq','delete(findobj(''tag'',''tas''));');

cid1 = get(ax1,'User');  % get cursor ID for 1st plot
cid2 = get(ax2,'User');  % get cursor ID for 2nd plot
plt('cursor',cid1,'set','xylim',[xmn xmx 9 ymx]);  % set plot 1 xy limits
