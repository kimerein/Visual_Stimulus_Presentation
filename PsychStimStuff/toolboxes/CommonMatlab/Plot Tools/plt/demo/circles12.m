...
plt(cN*r + repmat(c,N,1),'FigName','12 Circles (solution 3)',...
    'LineWidth',2,'Options','Ticks','Position',[10 sz(4)-715 620 648],...
    'Xlim',[-1.6  9.5],'Ylim',[-4.8 7.5],...
    'AxisPos',[1 1 1 1 1.2],'LabelY','','LabelX','',...
    'TraceID',reshape(sprintf('circle%2d',1:12),8,12)');
% Solutions #1 and 2 ----------------------------------------------
...
ID = [{'middle'; 'outer'}; repmat({'group1'},5,1); repmat({'group2'},5,1);];
...
h = plt(e,cN,'Position',[645 sz(4)-470 0 400],...
    'Figname','12 circles (part 2)','Options','Ticks Nocur -All','TraceID',['div ';'circ'],...
    'LabelX','','LabelY','','Xlim',[-1.1 1.1],'Ylim',[-1.1 1.1]);
set(gca,'xtick',[],'ytick',[]);                 % remove tick marks
hs = plt('slider',[.02 .1 .3],[1 -15 15],...    % create rotation speed slider
    'Rotation speed','',2,['%3d';'%3d';'%2d']);
bt = uicontrol('str','Stop','units','norm',...  % create close button
    'position',[.35 .02 .08 .05],'callback',...
    'delete(findobj(gcf,''str'',''Stop''))');   % the callback causes the button to delete itself
...
if ishandle(h(1)) plt('slider',hs,'set','visOFF'); end; % hide the slider
