...
% Output:
%      xmax.loc    (locations of local maxima of data in a structure array of dimensions channels/trials)
%          .zerodiff     (locations diff of data = 0 structure array of
%          dimensions channels/trials) % BA
...
xmax(1:C)=struct('loc',[]);
for ch=1:C,
         xmax(ch).zerodiff=[xmax(ch).loc; find(~diff(data(:,ch)))]; %%BA mod
   if nargin ==1
