...
%       Cerr  (Jackknife error bars for C - use only for Jackknife - err(1)=2)
    ...
    Cerr = [];confC = [];phistd = []; % initialized argout BA
...
%     error('Cerr computed only for Jackknife. Correct inputs and run
%     again'); % old BA
    warning('Cerr computed only for Jackknife.'); % BA (so you can always have same number of argsout
end;
if nargout > 7 && err(1)==0;
%     error('Errors computed only if err(1) is not equal to zero'); %old
    warning('Errors computed only if err(1) is not equal to zero'); %BA 
end;

% check_consistency(data1,data2); % OLD
check_consistency(data1,data2,1); % BA modified so that data1 and data2 can have different numbers of spikes different numbers of spikes 
                                  % seems like it is bug since one should
                                  % be able to enter data with different
                                  % numbers of spikes
