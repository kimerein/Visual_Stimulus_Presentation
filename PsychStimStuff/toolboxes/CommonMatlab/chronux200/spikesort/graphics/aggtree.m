...
% MODIFIED sbm, 4/19/04  New input syntax; data is unwrapped inside
%                        function now.  Previous syntax is below:
% ----------------------------------------------------------------------
%  AGGTREE  Generates a graphical summary of an aggregation procedure.
%      aggtree(tree, assignments, times)
% The input argument 'tree' describes the heirarchical aggregation and
%   its form is given in the comments for the 'aggregate' function.  The
%   'assignments' and 'times' inputs are required to provide summary
%   statistics for the final clusters.
% ----------------------------------------------------------------------
...
leaflabels(find(ypos ~= 0),:) = [];    % leafs have y pos == 0
...
for root = 1:length(treeroots)
    ...
    oldname = tree((find(atree(:,3) == treeroots(root))),1);
    if (isempty(oldname))
