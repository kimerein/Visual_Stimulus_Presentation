...
for rep = 1:opts.reps                                 % TOTAL # REPETITIONS
	...
	for iter = 1:opts.divisions                       % # K-MEANS SPLITS
        ...
        if (opts.progress)
            progressBar(0, 1, sprintf('Calculating %d means.', size(centroid,1))); % crude ...
        end
		...
		while (~(assign_converge | mse_converge))     % convergence?
			...
			clustdistsq = pairdist(waves, centroid, 'nosqrt', 'reuse');
            ...
            if (progress)
                progressBar(((M - changed_assigns)/M).^10, 5); % crude ...
            end
