...
for i=1:NCol
    ...
    %% hack to convert from unicode to normal ascii
    axgxdata(i).title = axgxdata(i).title(find(unicode2native(axgxdata(i).title,'')));
    switch(datatype)
        case 4
            axgxdata(i).data = fread(fid,np,'short') ;%%
            warning('UNTESTED axgx datatype import used');
        case 5
            axgxdata(i).data = fread(fid,np,'long') ;%%
            warning('UNTESTED axgx datatype import used');
        case 6
            axgxdata(i).data = fread(fid,np,'float') ;%%
            warning('UNTESTED axgx datatype import used');
        case 7
            axgxdata(i).data = fread(fid,np,'double') ;%%
        case 9 %% series
            value = fread(fid,1,'double'); %%
            incre = fread(fid,1,'double'); %%
            axgxdata(i).data = [value:incre:np*incre]';
