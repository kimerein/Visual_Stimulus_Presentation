function [output] = importStruct_abf(file, Sweeps,mode,HPFilter,FilterChns)

% function [output] = importStruct_abf(file, Sweeps,mode,HPFilter,FilterChns)
%
%% WARNING there is a gain problem with channel 2
%% possible causes:
%%          - Clampfit making file truncated in sweeps or length of sweep
%%          - Non-700B signal scaled differently.
%%
...
%     The input arguments:
%       Required:  
%               FILE - refers to the abf filename and it must be a string, including the file extension (e.g. 'patchexperiment.abf').
%               Sweeps -  refers to the episode number (among those available) the user wish to extract (e.g. 1, 2, 3,... ).
%                       -1 refers to all sweeps on file
%       Optional
%               mode -  refers to the whether should be output as single and scaled by gain and offset (mode 0) or as an int16 (mode1)
%               HPFilter - HighPass filter (2-pole Butterworth) cut-off in Hz.
%               FilterChns - channels to be filtered (-1 is default i.e. all channels)
%
...
%
%     
%     The output: is a struct including  
%               DATA,     contains the data matrix.  This data matrix has the same size as the data in the file,
%                           one row per channel.
%               
%     Modified by B. Atallah (bassam.atallah@gmail.com)
%     Based on  2002 - Michele Giugliano, PhD (http://www.giugliano.info)
...
%
%%% CHANGE LOG
% BA100406 changed read h.nLevelHysteresis  to      = fread(fid,1,'short');   % 1980
...
if nargin < 4  %% HighPass filtering
    bFilterHP = 0;
    HPFilter = 0;
    FilterChns =0;
elseif nargin <5
    FilterChns = -1; % default to filter all channels
end
...
N_sweeps = h.lActualEpisodes;      % Get the number of episodes contained in the data file...
if Sweeps == -1
    Sweeps = [1:N_sweeps];
end
if length(Sweeps)>1
    if Sweeps(2) == -1  % from specified sweep to end
        Sweeps = [Sweeps(1):N_sweeps];
    end
end
if (sum(Sweeps > N_sweeps)>0),        % and check for valid value of the user request.
...
N_samples    = h.lActualAcqLength / h.lActualEpisodes;
N_sweeps = size(Sweeps,2);%h.lActualEpisodes;
...
if mode == 1
    sdatatype = 'int16'; %save half the space
    bScaled = 0;
else
    sdatatype = 'single'; %float 32
    bScaled = 1;
end

bSparse = 0;
if bSparse
    data      = sparse(N_samples/Nchan,Nchan*N_sweeps+1,sdatatype);  %SPARSE
else
    data      = zeros(N_samples/Nchan,Nchan*N_sweeps+1,sdatatype);
end

 xdata = zeros(N_samples,1,sdatatype);
 xdata2 = zeros(N_samples/Nchan,Nchan,sdatatype);
 
 % Calculate dt from file
 tempdt = h.nADCNumChannels * h.fADCSampleInterval;
 dt = tempdt*1e-6;

 if HPFilter  %% define highpass filter.
     [B,A] = butter(2,2*HPFilter*dt,'high');
     bFilterHP = 1;
     if FilterChns == -1
         FilterChns = [1:Nchan];
     end
...
data(:,1) = (0:N_samples/Nchan-1)';

bFindLocInFile =1;
for i=1:size(Sweeps,2) %FOR all sweeps
    en = Sweeps(i);
    if i>1
        if (Sweeps(i)-Sweeps(i-1))>1
            bFindLocInFile =1;
        else
            bFindLocInFile =0;
        end
    end
    if (bFindLocInFile) %only run fseek when current sweep doesn't follow last
        if (h.fHeaderVersionNumber >= 1.6)
            if h.lNumScopes ==1
                fseek(fid, (h.size+s.size)+1394 + (en-1) * 2 * N_samples, 'bof');
            else
                fseek(fid, h.size + (en-1) * 2 * N_samples, 'bof'); %BA 031906 this allows it to read files saved in Clampfit
            end

        else
            fseek(fid, (2048+s.size)+370 + (en-1) * 2 * N_samples, 'bof'); % UNTESTED!
        end % if
        bFindLocInFile =0;
    end

    if (h.nDataFormat == 0) % Data representation. 0 = 2-byte integer; 1 = IEEE 4 byte float.
        xdata = fread(fid, N_samples, 'short');
    else
        xdata = fread(fid, N_samples, 'float');  %BA Doesn't work: Scaling is wrong
        if (h.lNumScopes ==1)
            error('importing abf (float) is not correctly implemented, save as abf(int) [h.nDataFormat = 1, ]');
        end
        if (mode == 1)
            error('Cannot run in mode =1 (integer) when importing abf (float) [h.nDataFormat = 1, ]');
        end
    end % if
    ...
    eval('xdata2 = reshape(xdata, Nchan, length(xdata)/Nchan)'';', '');     % This is taken from Mathworks' Tech-Note 1402

    for chan=1:Nchan
        if i==1 % BA so gain is calculated only once
            ADCchan = h.nADCSamplingSeq(chan) +1; %ADC channels start at 0  %BA
            gain(chan)   = h.fADCRange / h.lADCResolution / (h.fInstrumentScaleFactor(ADCchan)* h.x_fAutosampleAdditGain * h.fADCProgrammableGain(ADCchan) * h.fSignalGain(ADCchan));
%             if h.nTelegraphEnable(ADCchan) ==0
%                 gain(chan) = gain(chan)*h.x_fAutosampleAdditGain;
%             end
            offset(chan) = h.fSignalOffset(ADCchan) + h.fInstrumentOffset(ADCchan);
        end
    end % for
    if HPFilter  % do filtering
        xdata2(:,FilterChns) = filtfilt(B,A,xdata2(:,FilterChns)); 
    end

    if false %DEBUG BA
        x = 4;
        plot(xdata2(500:end,x))
        hold on;
        temp = max(xdata2(500:end,x));
%          temp = sprintf('maxB: %1.3f\tmaxR: %1.3f\nGain: %1.3f, Off: %1.3f\n %1.1f %1.1f %1.3f \n%1.1f %1.1f  %1.1f ', temp,temp2,gain(chan),offset(chan),h.fADCRange , h.lADCResolution ,h.fInstrumentScaleFactor(ADCchan) , h.x_fAutosampleAdditGain , h.fADCProgrammableGain(ADCchan) ,h.fSignalGain(ADCchan));
        title(temp);
        pause;
    end

    %report loading progress 
    if round(i/1000)== i/1000 %DEBUG
        i = i
    end
    ...
    if bSparse
        if (~mode)
            data(:,Nchan*(i-1)+1+1:Nchan*(i)+1) = sparse(xdata2 .* (gain'*ones(1,N_samples/Nchan))' + (offset'*ones(1,N_samples/Nchan))');
        else
            data(:,Nchan*(i-1)+1+1:Nchan*(i)+1) = sparse(xdata2);
        end
    else
        if (~mode)
            data(:,Nchan*(i-1)+1+1:Nchan*(i)+1) = xdata2 .* (gain'*ones(1,N_samples/Nchan))' + (offset'*ones(1,N_samples/Nchan))';
        else
            data(:,Nchan*(i-1)+1+1:Nchan*(i)+1) = xdata2;
        end
    end
...
for i =1:Nchan
    index = h.nADCSamplingSeq(i);
    tempADCChannelName{1,i} = char(h.sADCChannelName(10*index+1:10*index+10))';
    tempADCUnits{1,i} = char(h.sADCUnits(8*index+1:8*index+8))';
end
sADCChannelName = cell2struct(tempADCChannelName,'name',1);
sADCUnits = cell2struct(tempADCUnits,'name',1);

temp = num2str(h.lFileStartDate);
recDate = sprintf('%s-%s-%s',temp(end-3:end-2),temp(end-1:end),temp(1:4));
temp = max(strfind(file,'\'));
filename =  file(temp+1:end);
filepath =  file(1:temp);
output = struct('sfilename',filename,'sfilepath',filepath,'recDate',recDate,'sProtocol',h.sProtocolPath,'Nchan',Nchan,'Nsamples',...
    N_samples,'Nsweeps',N_sweeps,'xSweeps',Sweeps,'data',data,'dt',dt,...
    'gain',gain,'offset',offset,'bScaled',bScaled,'nADCSamplingSeq',h.nADCSamplingSeq,'sADCChannelName',sADCChannelName,'sADCUnits',sADCUnits,...
    'digSignalLowpass',h.fSignalLowpassFilter,'digSignalHighpass', h.fSignalHighpassFilter,...
    'FileVersionNumber',h.fFileVersionNumber,'sFileComment',h.x_sFileComment,'loadHighpass',HPFilter,'loadHighpassChn',FilterChns);
% Job done!
...
% h.nLevelHysteresis      = fread(fid,1,'int');   % 1980 %% BA100406
% changed to 'short' for at least 9.2.0.11
h.nLevelHysteresis      = fread(fid,1,'short');   % 1980
