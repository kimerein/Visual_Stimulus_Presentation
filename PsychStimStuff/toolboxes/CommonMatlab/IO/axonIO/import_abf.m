function [data N_sweeps Nchan N_samples dt gain offset] = import_abf(file, Sweeps,mode)

%   [DATA] = IMPORT_ABF(FILE, EN,  mode)
%
...
%     The second input argument Sweeps refers to the episode number (among those available) the user wish to extract (e.g. 1, 2, 3,... ).
%     The third input argument DT refers to the sampling interval of the acquired data (in units of seconds).
%
...
%   Fixed by B Atallah 
%       Reads Clampfit abfs
%       bSparse used sparse data structure
%   BA032006 calculate dt from nADCNumChannels and fADCSamplesInterval
%   BA032006 Mode 0 data is single and scaled, 
%            Mode 1 data is int16 and gain & offset hold scaling
%
%
...
% if nargin ==3
%     if dt * 1e6 ~= tempdt 
%         stemp = sprintf('User Entered dt = %1.2g, Calculated dt = %1.2g',dt,tempdt)
%         error(stemp);
%     end
% else
    dt = tempdt*1e-6;
...
for i=1:size(Sweeps,2) %FOR all sweeps
    ...
    if false %DEBUG BA
        x = 4;
        plot(xdata(500:end,x))
        hold on;
        temp = max(xdata(500:end,x));
%         temp = sprintf('maxB: %1.3f\tmaxR: %1.3f\nGain: %1.3f, Off: %1.3f\n %1.1f %1.1f %1.3f \n%1.1f %1.1f  %1.1f ', temp,temp2,gain(chan),offset(chan),h.fADCRange , h.lADCResolution ,h.fInstrumentScaleFactor(ADCchan) , h.x_fAutosampleAdditGain , h.fADCProgrammableGain(ADCchan) ,h.fSignalGain(ADCchan));
    ...
    %count sweeps
    if round(i/10)== i/10 %DEBUG
        i = i
    end
    %     if round(i/50)== i/50 %DEBUG
    %         pack ('temppack.data')
    %     end
...
h.fTelegraphAdditGain   = fread(fid,16,'float');     % 4576  BA031906  Does NOT seem to read correct value from clampex 9.2.0.11. 
h.fTelegraphFilter      = fread(fid,16,'float');     % 4640  BA as above
