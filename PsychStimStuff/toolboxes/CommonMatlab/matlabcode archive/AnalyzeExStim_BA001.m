...
WOI = [500e-6 500e-6; 2500e-6 2500e-6];
...
extracted_spikes_Intra = zeros(int16(Waveform_size(1)),250,size(filenumbers,2));
extracted_spikes_Extra = zeros(int16(Waveform_size(2)),250,size(filenumbers,2));
...
for (i =1: size(filenumbers,2))
    ...
    data_GT0 = find(intraCell_data > 0);

    
    % End & beg of all potential spike
    data_GT0_shift = circshift(data_GT0,1);
    data_GT0_shift(size(data_GT0_shift,1),1) = 0;
    index_end_spike = data_GT0(find ( data_GT0 - data_GT0_shift > 1)-1);

    index_beg_spike = data_GT0(find ( data_GT0 - data_GT0_shift > 1));
    index_beg_spike = circshift(index_beg_spike,1);
    index_beg_spike(1) = data_GT0(1);
    % find length of spike above 0
    length_spike = index_end_spike - index_beg_spike;%lenth in index
   
    % find peak
    index_max = max(intraCell_data(index_beg_spike:index_end_spike))
    index_beg_conf_spike_1 = index_beg_spike(find(length_spike > (minpeakwidth * Ts )& length_spike < (maxpeakwidth * Ts ) ));
    % this is a poor criteria because a shift in the baseline changes how
    % much of the spike is above 0
    
    % find Spike interval (beg to beg of Intracellular spike)
    %NOT used as criteria for a spike
    ...
    for (ii  =1:size(index_beg_conf_spike_1,1))
        extracted_spikes_Intra(:,ii+2,i) = intraCell_data (index_beg_conf_spike_1(ii,1)-WOI(1)* Ts:index_beg_conf_spike_1(ii,1)+WOI(2)* Ts)';
        extracted_spikes_Extra(:,ii,i) = extraCell_data (index_beg_conf_spike_1(ii,1)-WOI(3)* Ts:index_beg_conf_spike_1(ii,1)+WOI(4)* Ts)';
    end
    % extracted_spikes_Intra(:,2,i) = time_data(1:Waveform_size);
end
figure(1)
