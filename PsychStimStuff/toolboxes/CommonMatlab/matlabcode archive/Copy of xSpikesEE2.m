function [Xspikes spikeData sumspiketimeSw skipped] = xSpikesEE2(index_beg_spike,data,Chn_ind,Ts,N_chn,maxpeakamp,maxpeakwidth,neighbors_ind,WOI)
...
N_samples = size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),1)*N_chn;
lastsweepnum =0;
if ~isempty(index_beg_spike)
    ...
    for (ii  = 1:size(index_beg_spike,1)) % for all spikes found
        ...
        if (temp == sweepnum) %check window around index_beg_spike exists in the same sweep
            ...
            if(temp2 < size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),1)*size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),2))
                ...
                if( min(temp1)> maxpeakamp) %
                    ...
                    if (mod(Peak1(1,1),size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),1))/Ts > WOI(1) & (size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),1)- mod(Peak1(1,1),size(data(:,indexoffset+nIntracellsignals+Chn_ind:N_chn:end),1)))/Ts> WOI(2)) %check if WOI exists around spike (if to early or late in sweep it may not
%                         spiketime = index_beg_spike(ii,1);
                        ...
                        if (timeInsweep > 0 )
                            ...
                            %WAVEFORM
                            % assume 2 neighbors
                            Xspikes(:,Nextactedspikes,z) = [full(data(Peak1(1,1)-WOI(1)* Ts:Peak1(1,1)+WOI(2)* Ts,indexoffset+nIntracellsignals+Chn_ind:N_chn:end));...
                                data(Peak1(1,1)-WOI(1)* Ts:Peak1(1,1)+WOI(2)* Ts,indexoffset+neighbors_ind(1):N_chn:end);...
                                data(Peak1(1,1)-WOI(1)* Ts:Peak1(1,1)+WOI(2)* Ts,indexoffset+neighbors_ind(2):N_chn:end)];
