...
subplot(1,2,1)
temp = 1;
plot(time_data,data(:,temp));
hold all;
stemp = sprintf('%s    Sw# %d\nThreshold= %d\n Samp= %d   Ts(Hz)= %5.0f',processedFiles,temp,threshold,N_samples/N_chn,Ts );
ylim([min(data(5000:end-50,1)) max(data(5000:end-50,1))]);
...
temp = dataIC2(500:end,1) > .5;  % 500pA; %skip beginning because of garbage (unknown why)
temp = diff(temp);
...
stemp = sprintf('%s',processedFiles);
title(stemp,'Interpreter','none');
ylabel('Sweep #');
xlabel('Time (ms)');
%Interval
line(Interval/Ts,[max(spiketimeSw(:,1))-1 max(spiketimeSw(:,1))-1],'Color','r','LineWidth',3);
...
figure(figID+3);
plot([1:size(cumspiketime,2)]/Ts,cumspiketime)
stemp = sprintf('%s',processedFiles);
title(stemp,'Interpreter','none');
ylabel('Cum Spikes');
xlabel('Time (ms)');
%Interval
line(Interval/Ts,[max(cumspiketime)-1 max(cumspiketime)-1],'Color','r','LineWidth',3);


%% histogram in bins

%             sweepNum = 1+ floor(index_beg_spike(iii)/samplePerSweep);
%             sampleNum = index_beg_spike(iii) - (sweepNum-1)*samplePerSweep;

%% intantaneous frequency  with pclamp is that faster?

%% plot all the data from the experiment.. see obvious trends.
