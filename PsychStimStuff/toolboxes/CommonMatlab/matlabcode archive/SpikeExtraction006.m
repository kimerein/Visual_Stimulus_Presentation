...
fileheader = ['2005_07_15_'];
Nfiles_per_header = [10];
PROTOCOL = 1; %ICP = 1, ICR = 2
filenumbers = [18 25 30 42 48 59 64 71 78 86]; %file index for day
CELL_NUM = [07150501 07150502 07150503 07150504 07150505 07150506 07150507 07150508 07150509 07150510]; %dateCell# 
PUT_TYPE = [2 1 1 3 1 1 1 1 3 1];% 1= PYR, 2 = Regular spiking 3 = Fast Spiking 0 = Other

%INPUT File parameters
dirheader =  'e:\My Documents\Academic\Rotation\Scanziani\Ascii\';
fileheader = ['2005_07_12_';'2005_07_13_';'2005_07_14_';'2005_07_15_'];
Nfiles_per_header = [3;4;6;10];
PROTOCOL = 1; %ICP = 1, ICR = 2
...
filenumbers = [4 11 18 ...
   17 32 40 49 ...
   6 12 24 36 47 56 ...
   18 25 30 42 48 59 64 71 78 86]; %file index for day
CELL_NUM = [07120501 07120502 07120503 07130501 07130502 07130503 07130504 07140501 07140502 07140503 07140504 07140505 07140506 07150501 07150502 07150503 07150504 07150505 07150506 07150507 07150508 07150509 07150510]; %dateCell# 
PUT_TYPE = [1 2 1 1 2 2 2 1 1 1 1 1 2 2 1 1 2 1 1 1 1 2 1];% 1= PYR, 2 = Regular spiking 3 = Fast Spiking 0 = Other
    ...
    for (i =filenumber_ind:filenumber_ind + Nfiles_per_header(j,1) -1)  % count through filenumbers for this fileheader
      ...
      %             Peak1 = find(intraCell_data (index_beg_spike(ii,1):index_beg_spike(ii,1)+ 80) == max(temp1)) + index_beg_spike(ii,1);
                   Peak1 = find(temp1 == max(temp1)) + index_beg_spike(ii,1);
                    ...
                    %take diff and fond Trough
                    temp1 = diff(intraCell_data (index_beg_spike(ii,1)-30:index_beg_spike(ii,1)+ 50)');
                    PeakDiff_ind = find( temp1 == max(temp1)) + index_beg_spike(ii,1);
                    extracted_spikes(1:int16(Waveform_size(1)),IndexES,z+4) = [diff(intraCell_data(PeakDiff_ind(1,1)-WOI(1)* Ts:PeakDiff_ind(1,1)+WOI(2)* Ts))'; 0];
                ...
                extracted_spikes(1,IndexES,z+1) = HEADER_VERSION;
          ...
          %{
                ...
                extracted_spikes(19,IndexES,z+1) = CONFIG;
...
%scale intracellular derivative.
scal = 1./max(extracted_spikes(:,:,5));
Scalar =  ones(size(extracted_spikes,1),1)*scal;
extracted_spikes(:,:,5) = extracted_spikes(:,:,5).*Scalar ;
extracted_spikes(63,:,2) = scal(1,:); %Record factor in header


putative_type = [putative_type PUT_TYPE];
cell_number = [cell_number CELL_NUM];

save(savefilename,...
    'extracted_spikes', 'total_extractedspikes', 'ID_SpikeLast', 'processedFiles', 'Nspikes_in_File', ...
    'putative_type', 'cell_number', '-mat');
%{
Example File:
