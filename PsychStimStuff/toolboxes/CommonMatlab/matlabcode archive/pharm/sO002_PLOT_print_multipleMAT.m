...
spiketime = totalspiketime;
N_sweepsExtracted = totalN_sweepsExtracted;
figID = 100;
...
figure(figID+10)
binsize = 1000; %(ms)
binned = bin(spiketime(:,1),(N_samples/N_chn * N_sweepsExtracted)/ Ts/(binsize*1e-3))/(binsize*1e-3);
tmean = mean(binned);
tstdev = std(binned);
...
if true
    % data in SWEEPS
    data = [644 1364 1623 1973 2355 2915]
...
if true
    figure(figID +100)
    % data in sec
    data = [300 500; 800 1000;1570 1670;1750 1950;2175 2375; 2700 2900;3150 3229]
    for(i=1:size(data,1))
