...
filenumbers = [0]; %file index for day
...
for (j = 1: size(fileheader,1))
   filenumber_ind = i+1;
    for (i =filenumber_ind:filenumber_ind + Nfiles_per_header(j,1) -1)  % count through filenumbers for this fileheader
        ...
        temp = textscan(fid,'%s""','delimiter','\n'); %if I don't make it into a string the cell have '' rather then are empty... (dont' know why)
        ...
        for ii = 1: N_chn
            ...
            Chn_name(ii,1:ln) = a(1,1:ln)  %how do I copy strings
%             if ( isempty(regexp(Chn_name(ii,:),'1')) ~= 1)
            if ( ii == 2) %for will's data
                Intra_Chn = ii
            end
            if ( isempty(regexp(Chn_name(ii,:),'ff')) ~= 1)
                Extra_Chn = ii
                nExtra_Chn = nExtra_Chn +1
            end
        end
        ...
        if (nExtra_Chn ~= 0)
            ...
            extraCell_data = zeros(size(my_data,1),Extra_Chn+1:N_chn:end,my_data_zind) ;
        end
      ...
      %             Peak1 = find(intraCell_data (index_beg_spike(ii,1):index_beg_spike(ii,1)+ 80) == max(temp1)) + index_beg_spike(ii,1);
                   Peak1 = find(temp1 == max(temp1)) + index_beg_spike(ii,1);
                    ...
                    temp1 = diff(intraCell_data (index_beg_spike(ii,1)-round(.6e-3*Ts):index_beg_spike(ii,1)+ round(1e-3*Ts))');
                    ...
                    if (nExtra_Chn ~= 0)
                        Intra_cell = 0;
                        Chn = Extra_Chn;
                        temp2 = extraCell_data (index_beg_spike(ii,1)-round(1e-3*Ts):index_beg_spike(ii,1)+ round(.6e-3*Ts))';
                        Trough1 = find(extraCell_data (index_beg_spike(ii,1)-round(1e-3*Ts):index_beg_spike(ii,1)+ round(.6e-3*Ts)) == min(temp2)) + index_beg_spike(ii,1)-round(1e-3*Ts);
                        %take first Trough
                        ...
                        extracted_spikes(1: int16(Waveform_size(1)),IndexES,z) = zeros(1,int16(Waveform_size(1)))';
