function     [selectDataNum sselectData selectData selectTs] = readMaxSpikeData(readdirheader,path)
% function     [selectDataNum sselectData{i,1} selectData{i,1} selectTs] = readMaxSpikeData(path)
% INPUT path to run dir on
% DESC: reads all files in that match dir "path"
% returns   selectData: the mean data for the largest spike from  each
%               files 
%           sselectData: sting describing data
%           selectDataNum: number of sweeps in each mean
%           selectTs : sampling freq
filedata = struct2cell(dir(strcat(readdirheader,path)));
...
selectDataNum  = size(spike_ind,2);%number of sweeps in mean
sselectData =  temp;
selectData =  Data;
if ~exist('Ts')
    Ts = NaN;
