...
% NOTE: this truncates data about spike too (i.e. columns 2 and 4)
anal_spikes = extracted_spikes(1:110,:,:);
 % Analyze a subset of spikes
%Indices of Spikes to analyze
% Filename = [07120502 07130504 07130503 07130501 07150508 07150504 07180503]; %Group B
...
figure(figure_ind)
subplot(2,1,1)
plot(extracted_spikes(:,:,Z1))
subplot(2,1,2)
plot(extracted_spikes(:,:,Z2))
graphTitle = '';
%HISTOGRAM
close all;
figure(figure_ind +1)
...
[Bin_Value Bin_Location]  = hist(extracted_spikes(:,:,Z1)',800);
image(Bin_Location,-[1:size(Bin_Value,2)],Bin_Value)
subplot(2,1,2)
clear Bin_Value; clear Bin_Location;
[Bin_Value Bin_Location]  = hist(extracted_spikes(:,:,Z2)',800);
image(Bin_Location,-[1:size(Bin_Value,2)],Bin_Value)
...
%%% Z1
 [PC, SCORE1, LATENT, TSQUARE] = princomp(extracted_spikes(:,:,Z1)');
 figure(figure_ind +7)
...
%%%%  Z2
[PC, SCORE2, LATENT, TSQUARE] = princomp(extracted_spikes(:,:,Z2)');
 figure(figure_ind +6)
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NClusters = 2;
% clusters = kmeans(extracted_spikes(:,:,Z1)',NClusters);
% %for i=NClusters:-1:1
% graphTitle = 'Intracellular';
% for i=1:NClusters
%     clear Ecluster; clear cluster_ind; clear EcCell;
%     cluster_ind = find(clusters == i); %EXTRACT CLUSTER
%     Ecluster = extracted_spikes(:, cluster_ind,Z1);
%     EcCell = extracted_spikes(3, cluster_ind,Y1);
...
% NClusters = 2;
% clusters = kmeans(extracted_spikes(:,:,Z2)',NClusters);
...
%     clear Ecluster; clear cluster_ind; clear EcCell;
%     cluster_ind = find(clusters == i); %EXTRACT CLUSTER
%     Ecluster = extracted_spikes(:, cluster_ind,Z2);
%     EcCell = extracted_spikes(3, cluster_ind,Y2);  %%%%%%%%%%%%%%%%%
%     uniq_Cells = unique(EcCell)  ;
%     Eplot = find(extracted_spikes(3,:,Y2)  == uniq_Cells(1,i));
%     beg_ind = min(Eplot);
%     end_ind = max(Eplot);
%     % %plot of Waveforms from clusters
%     figure(figure_ind+10)
