function [processedFiles,Nspikes_in_File,extracted_spikes] = xSpikes(readfileheader,readfilenumbers)
%***********************************************************************
% Function
...
for (j = 1: size(readfileheader,1))  %LOOP through headers
    ...
    for (i =filenumber_ind:filenumber_ind + Nfiles_per_header(j,1) -1)  % count through readfilenumbers for this readfileheader
        ...
        filepath = strcat(readdirheader,filename) ;
        ...
        clear my_data;
        [my_data(:,:,my_data_zind) N_sweeps] = import_abf(filepath,-1,1/Ts,xChan);
        N_samples = size(my_data,1);

        %%% Plot abf files
        figure(figure_ID+i);
        for jj = 1:size(xChan,2)
            subplot(2,ceil(size(xChan,2)/2),jj);
            plot(my_data(:,jj+1:size(xChan,2):end),'r-');
            xlim([3500,4000])
            if jj ==1 %IntraAxis
               ylim([-100, 50])
            else %ExtraAxis
            ...
            hold on;
            stemp = sprintf('%s Sweeps= %d',filename,N_sweeps);
        ...
        if (nExtra_Chn ~= 0)
            ...
            extraCell_data = intraCell_data .* 0 ;
        end
        % Find spike from INTRAdata
        data_GT = intraCell_data > threshold;
