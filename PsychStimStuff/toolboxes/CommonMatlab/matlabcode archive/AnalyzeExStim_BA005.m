...
for (i =1: size(filenumbers,2))
   ...
   fid = fopen(file,'r');
    ...
    clear intraCell_data; clear extraCell_data;
    time_data = my_data(:,1);
    intraCell_data =my_data(:,Intra_Chn+1:N_chn:end) ;
    extraCell_data = my_data(:,Extra_Chn+1:N_chn:end) ;
    %clear my_data;
 ...
 % VER 001
        ...
        %extracted_spikes(3,IndexES,z+1) = 0;%* filename
        extracted_spikes(4,IndexES,z+1) = CELL_NUM(1,i);
         extracted_spikes(5,IndexES,z+1) = PROTOCOL;
        extracted_spikes(6,IndexES,z+1) = Ts;
        extracted_spikes(7,IndexES,z+1) = Chn;
        extracted_spikes(8,IndexES,z+1) = Intra_cell;
        extracted_spikes(9,IndexES,z+1) = Sweep;
        extracted_spikes(10,IndexES,z+1) = Sweep_Times(Sweep); %time in sec from beginning of experiment
        extracted_spikes(11,IndexES,z+1) = Timeofspike;
        extracted_spikes(12,IndexES,z+1) = interspike_interval(ii,1);
        extracted_spikes(13,IndexES,z+1) = N_samples;
...
% Scale factor
    ...
    %Adjust baseline
    temp = -60 - mean(extracted_spikes(1:10,:,1));
    Baseline = ones(size(extracted_spikes,1),1)*temp;
