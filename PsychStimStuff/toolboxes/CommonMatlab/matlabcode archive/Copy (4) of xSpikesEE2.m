...
if nargin < 10
    WOI = [750e-6 750e-6; 2000e-6 2000e-6]; % left for backward compatibility
end
...
if isinteger(data)
    Xspikes = zeros(int32(((WOI(1)+WOI(2))*Ts+1)*(n_neighbors+1)),size(index_beg_spike,1),'int16');
else
    Xspikes = zeros(int32(((WOI(1)+WOI(2))*Ts+1)*(n_neighbors+1)),size(index_beg_spike,1),'single');
end
...
if ~isempty(index_beg_spike)
    ...
    for (ii  = 1:size(index_beg_spike,1)) % for all spikes found
        ...
        if ((temp == sweepnum & temp2 == sweepnum)) %check window around index_beg_spike exists in the same sweep
            ...
            temp1 = single(data ((electrode_index_beg_spike-PreWindow*Ts):temp2))';
            ...
            if bAllEE %% align to largest signal if triggers came from all electrodes
                for(w=1:n_neighbors) % find the largest signal (relative to std)
                    ...
                    temp1 = single(data((neighbor_index_beg_spike_index(w)-PreWindow*Ts): wtemp(w)))';
            ...
            min_index = min_index + PreWindow*Ts;
            %             if((Amp*-1)> maxpeakamp) %
...
%                             a = ii
%                         end
                        temp1 = data((Peak1(1,1)-WOI(1)* Ts):(Peak1(1,1)+WOI(2)* Ts));
                        %% UNTESTED
                        for n  = 1: n_neighbors
                            temp = data(neighbor_index_peak_spike_index(n)-WOI(1)* Ts:neighbor_index_peak_spike_index(n)+WOI(2)* Ts);
                            temp1 = [temp1 temp];
                        end
...
stemp = sprintf('EE:\t%d\nNeighbors:\t%s\nSpikesX:\t%d of %d \tSkipped: %d,%d,%d,%d\n\t%1.3f sec',Chn_ind,num2str(neighbors_ind),...
