function [Xspikes spikeData sumspiketimeSw skipped] = xSpikesEE2(index_beg_spike,data,Chn_ind,Ts,N_chn,maxpeakamp,maxpeakwidth,neighbors_ind,nIntracellsignals,WOI,std)
...
if ~isempty(index_beg_spike)
    ...
    for (ii  = 1:size(index_beg_spike,1)) % for all spikes found
        ...
        if ((temp == sweepnum & temp2 == sweepnum)) %check window around index_beg_spike exists in the same sweep
            ...
            temp1 = single(data ((electrode_index_beg_spike-0.5e-3*Ts):temp2))';
            Amp = abs(min(temp1))/std(Chn_ind);
            align_chn = Chn_ind;
            if false %DEBUG
                plot(temp1);
                pause;
            ...
            if bAllEE %% align to largest signal if triggers came from all electrodes
                for(w=1:size(neighbors_ind,2)) % find the largest signal (relative to std)
                    neighbor_index_beg_spike_index(w) = indexNeighbor(w) + indexb + index_beg_spike(ii,1);
                    wtemp(w) = neighbor_index_beg_spike_index(w)+ 1e-3*Ts;  % min in spike must occur within 1ms of crossing threshold
                    temp1 = single(data((neighbor_index_beg_spike_index(w)-0.5e-3*Ts): wtemp(w)))';
                    temp_index(w) = min(temp1);
                    wtemp(w) = abs(temp_index(w))/std(neighbors_ind(w));
            ...
            if((Amp*-1)> maxpeakamp) %
                ...
                Peak1 = min_index + electrode_index_beg_spike;
                if( Peak1 - lastPeak1 )> Ts*1e-3   % must be at least 1ms since last spike
                    for i=1: size(neighbors_ind,2)
                        neighbor_index_peak_spike_index(i) = indexNeighbor(i) + indexb + index_beg_spike(ii,1) + min_index;
                    end
                    ...
                    if (timeInsweep > 0 ) %% (what is this condition?
                        ...
                        try  %for DEBUGGING easily
                            ...
                            %                                                         a = ii
                            %                                                     end
                            wave(1,:) = data(Peak1(1,1)-WOI(1)* Ts:Peak1(1,1)+WOI(2)* Ts);
                            wave(2,:) = data(neighbor_index_peak_spike_index(1)-WOI(1)* Ts:neighbor_index_peak_spike_index(1)+WOI(2)* Ts);
                            wave(3,:) = data(neighbor_index_peak_spike_index(2)-WOI(1)* Ts:neighbor_index_peak_spike_index(2)+WOI(2)* Ts);

                            % MUST BE MANUALLY CHANGED for more Neighbors
                            Xspikes(:,Nextactedspikes,z) = [wave(1,:) wave(2,:) wave(3,:)];
                            skipped(ii) =0;
                            lastPeak1 = Peak1;
                            lastAmp = Amp;
                            if (mod(ii,1000) == 0)
