% [m xBins yBins sd stderr] = MeanSmooth2(x,y,z,xBins, yBins)
...
% the relevant bin.
%
% see also MeanSmooth, MedianSmooth

function [m, xBins, yBins, sd, stderr] = MeanSmooth(x,y,z,xBins,yBins)
...
sd = zeros(nxBins,nyBins,nDim);
stderr = zeros(nxBins,nyBins,nDim);
...
for nx=1:nxBins, 
	for ny=1:nyBins
		...
		if ~isempty(PointsInBin)
			m(nx,ny,:) = mean(z(PointsInBin,:));
			sd(nx,ny,:) = std(z(PointsInBin,:));
			stderr(nx,ny,:) = sd(nx,ny,:) / sqrt(length(PointsInBin));
		end
	end
end
