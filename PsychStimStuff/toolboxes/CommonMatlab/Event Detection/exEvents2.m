function [outdata df]= exEventsF(data,dt,fid,th,dfilter,bplot)
% function [outdata df]= exEventsF(data,dt,fid,th,dfilter,bplot)
...
WOId = [8 8].*round((1/dt/1000)); % (samples) too look for first positive derivitive
frac = 0.1; % fraction of peak to find
% outdata = findFracPeak(df,ind_pk,frac,WOId); % get parameters of event
