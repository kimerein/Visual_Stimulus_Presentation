...
%% Baseline search parameter
basWinDelay = 10;% delay back before (ms) trough/peak to search for baseline
basWinDelay = 1/output.dt/1000*basWinDelay;
basWin= 2;% window before (ms) trough/peak to search for baseline
basWin= 1/output.dt/1000*basWin;

perTrough=5; %percentage of Peak/trough to find.
%%%%

% prodata = prodata-ones(size(prodata,1),1)*max(prodata); %% for troughs makes everything less then zero
    thres= -100
    a= prodata < thres;
    ...
    for (i = 2: size(a,1)*size(a,2))
        if a(i) > 0 & a(i-1) <= 0 % if trough
            ...
            baseline = mean(prodata((trough(j)-(basWinDelay+basWin)):(trough(j)-(basWinDelay))))
            temp = prodata-perTrough/100*(abs(prodata(trough(j)))-abs(baseline)) ;
             %% problem in axis is crossed
            %find baseline
