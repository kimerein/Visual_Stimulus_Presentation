                    ...
                    for jjjj=1:(size(temp_spikeData{j},1))
                        ...
                        if  sweepnum > lastINSW 
                            ...
                            if (temp2-int32(Ts*10e-3)) <1  % BEGINNING
                                ...
                                kk = kk+1;
                                temp1(i1,:) =  [IntraCellData(1)*ones(1,abs(temp2-int32(Ts*10e-3))+1,'int16') IntraCellData((1):(temp2 + int32(Ts*50e-3)))];
                            elseif (temp2 + int32(Ts*50e-3)) > size(IntraCellData,1)*size(IntraCellData,2) % END
                                spikeIntraDataMissing(kk) = temp2;
                                kk = kk+1;
                                temp1(i1,:) =  [IntraCellData((temp2-int32(Ts*10e-3)):end) [IntraCellData(end)*ones(1,(temp2+int32(Ts*50e-3))-size(IntraCellData,1)*size(IntraCellData,2),'int16')]];
                            else
                                temp1(i1,:) = IntraCellData((temp2-int32(Ts*10e-3)):(temp2 + int32(Ts*50e-3)));
...
%                                 jjjj;
%                             end
                        else  %%% intracellular signal at -60mV (or INreversal)
                            tempbINSW{j}(jjjj)=1;
                            %
                            % catch cases where intradata doesn't
                            % exist(beggining or end of file)
                            if (temp2-int32(Ts*10e-3)) <1  % BEGINNING
                                spikeIntraDataMissing0(kk) = temp2;
                                kk = kk+1;
                                temp0(i0,:) =  [IntraCellData(1)*ones(1,abs(temp2-int32(Ts*10e-3))+1,'int16') IntraCellData((1):(temp2 + int32(Ts*50e-3)))];
                            elseif (temp2 + int32(Ts*50e-3)) > size(IntraCellData,1)*size(IntraCellData,2) % END
                                spikeIntraDataMissing0(kk) = temp2;
                                kk = kk+1;
                                temp0(i0,:) =  [IntraCellData((temp2-int32(Ts*10e-3)):end) [IntraCellData(end)*ones(1,(temp2+int32(Ts*50e-3))-size(IntraCellData,1)*size(IntraCellData,2),'int16')]];
                            else
                                temp0(i0,:) = IntraCellData((temp2-int32(Ts*10e-3)):(temp2 + int32(Ts*50e-3)));
                    ...
                    IntraCHN = 1;
                    if i1 >1
                        temp_IntraEX{j,IntraCHN} = single(temp1(1:i1-1,:))*single(output.gain(IntraCHN))+ single(output.offset(IntraCHN));
                    else
                        temp_IntraEX{j,IntraCHN} = zeros(1,size(temp1,2))
                    end
                    if i0 >1
                        temp_IntraIN{j} = single(temp0(1:i0-1,:))*single(output.gain(IntraCHN))+ single(output.offset(IntraCHN));
                    else
                        temp_IntraIN{j} = zeros(1,size(temp0,2))
                    end
                    ...
                    %                                        hold on;
                    hold off
                    temp = mean(temp_IntraEX{j},1) - mean(mean(temp_IntraEX{j},1)) + 10;
                    plot([1:size(temp1(jjjj,:),2)]*1e3/Ts,temp,'-b');
                    hold on
                    temp = mean(temp_IntraIN{j},1) - mean(mean(temp_IntraIN{j},1));
     ...
     if breport
         ...
         %     print('-dtiffn',temp);
         save(temp,'sourcefile','EEgroup','klusters','Center','num_c','temp_spikeData','temp_IntraIN','temp_IntraEX','tempbINSW','spikes_to_plot','unit_Nspikes','std_threshold','minTime')  %% NOTE font size too big on axis (overlaps)
