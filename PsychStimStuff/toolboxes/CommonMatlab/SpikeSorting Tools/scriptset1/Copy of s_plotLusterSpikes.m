...
%             unit_spiketimesX = single(spikeData(klusters==j,1));
            ...
            if nIntracellsignals >0
                ...
                for jjj=1:nIntracellsignals
                    ...
                    temp1 = zeros(min(size(temp_spikeData{j},1),1000),int32(Ts*60e-3)+1);
                    for jjjj=1:min(size(temp_spikeData{j},1),1000)
                        ...
                        if temp2
                            ...
                            if (temp2-int32(Ts*10e-3)) <1  % BEGINNING
                                ...
                                kk = kk+1;
                                temp1(jjjj,:) =  [temp(1)*ones(1,abs(temp2-int32(Ts*10e-3))+1,'int16') temp((1):(temp2 + int32(Ts*50e-3)))];
                            elseif (temp2 + int32(Ts*50e-3)) > size(temp,1)*size(temp,2) % END
                                spikeIntraDataMissing(kk) = temp2;
                                kk = kk+1;
                                temp1(jjjj,:) =  [temp((temp2-int32(Ts*10e-3)):end) [temp(end)*ones(1,(temp2+int32(Ts*50e-3))-size(temp,1)*size(temp,2),'int16')]];
                            else
                                temp1(jjjj,:) = temp((temp2-int32(Ts*10e-3)):(temp2 + int32(Ts*50e-3)));

                            end

                        end
                    end
                    temp_Intra{j} = single(temp1)*output.gain(jjj)+ output.offset(jjj);
                    hold on;
                    plot([1:size(temp1(jjjj,:),2)]*1e3/Ts,temp_Intra{j},'-k');
                    %                        plot([1:size(temp1(jjjj,:),2)]*1e3/Ts,(mean(temp1,1)+std(single(temp1),1))*output.gain(jjj),'--k');
                    temp = mean(temp_Intra{j},1);
                    plot([1:size(temp1(jjjj,:),2)]*1e3/Ts,temp,colororder2(jjj,:));
                    axis tight
