function param = xSpikeParams(data,dt,bsmooth)
% function param = xSpikeParams(data,dt)
...
%% respectively.
param = zeros(size(data,1),10);
for i = 1:size(data,1)
    ind_t1 = find(data(i,:) == min(data(i,:)'),1,'first');
    ind_p1= find(data(i,:) == max(data(i,1:ind_t1)),1,'first');
    ind_p2= find(data(i,ind_t1:end) == max(data(i,ind_t1:end)),1,'first') + (ind_t1-1);
...
%     end
  ...
  try
    ...
    ind_P2F = ind_p2 -1 + find(abs(data(i,ind_p2:end)-P2*.5) == min(abs(data(i,ind_p2:end)-P2*.5)),1,'first') ;
    ...
    if debug  %% show parameters on plot
        ...
        if size(dt,1) ==1
            param(i,:) = [P1 T1 P2 P1W*step*dt T1W*step*dt P2W*step*dt T1F*step*dt T1R*step*dt P2R*step*dt P2F*step*dt];
        else
            param(i,:) = [P1 T1 P2 P1W*step*dt(i,1) T1W*step*dt(i,1) P2W*step*dt(i,1) T1F*step*dt(i,1) T1R*step*dt(i,1) P2R*step*dt(i,1) P2F*step*dt(i,1)];
        end
  catch
      i
  end
