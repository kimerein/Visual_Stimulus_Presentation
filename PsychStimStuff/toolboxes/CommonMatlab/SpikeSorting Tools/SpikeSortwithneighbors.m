...
WOI = [500e-6 500e-6; 1500e-6 1500e-6];%Window of interest around Beg_spike -1000us + 2000us
std_threshold = -3; %number of std for threshold
b_intra = 0; % trigger on negative going spike
maxpeakamp = 10000*std_threshold;
maxpeakwidth =  2e-3;  
...
for triggerEE=nIntracellsignals+1:N_chn  % all Extracellular signals, threshold
    ...
    fmean(triggerEE) =  mean(mean(single(output.data(:,indexoffset+triggerEE:N_chn:end))));
     ...
     stemp = sprintf('Spike Detection Parameters\tSTD:\t%1.3f, M:\t%1.3f,\tTH:%d',fstd(triggerEE),fmean(triggerEE),std_threshold)
    index_beg_spike = [index_beg_spike int32(FindSpikes(output.data(:,indexoffset+triggerEE:N_chn:end), std_threshold*fstd(triggerEE) ,Ts,b_intra))];
end
    index_beg_spike = unique(index_beg_spike);
    %Extract spikes
    %  it doesn't matter which triggerEE is used since spikes are from all
    %  electrodes
    [allElectrodesXspikes spikeData temp skipped]  = xSpikesEE2(index_beg_spike,output.data,triggerEE,Ts,N_chn,maxpeakamp,maxpeakwidth,neighbors,nIntracellsignals,WOI);
    for jj = 1:4
...
%         temp = sprintf('%s%sEE%d',spath,figdesc,triggerEE);
        ...
        save(temp,'XspikeParams','neighbors','allElectrodesXspikes','spikeData','Spikeskipped');
    end
...
figure(10)
set(gcf,'Position',[700 0 700 1120],'OuterPosition',[0 0 0 0])
subplot(2,2,1)
...
title('Overclustered in PCA1 vs PCA2');
...
if bsave
    ...
    temp = sprintf('%s%sEE%d',spath,figdesc,triggerEE);
    %     print('-dtiffn',temp);
    save(temp,'Cluster','Center','num_c')  %% NOTE font size too big on axis (overlaps)
end
if breport
    dirtemp = 'REPORT';
    figdesc = 'Cluster';
...
% unit = cell(n_units,1);
% % unit{1} = [1:7 9:23] % A
% unit{1} = [1 28 13 30 22 6 26 30 17 8 27 15 21 7 14] % A
%  unit{2} = [5 23 12]%B
%  unit{3} = [9]%C
%  unit{4} = [2]%D
%  unit{5} = [29]%E
%  unit{6} = [20 18 19 11 4 25 10]%F
% unit{7} = [24]%G
% unit{8} = [16]%H
% unit{9} = [18]%I
% unit{10} = [19]%J
