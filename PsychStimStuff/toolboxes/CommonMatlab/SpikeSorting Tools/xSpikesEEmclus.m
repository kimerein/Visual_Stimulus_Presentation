function [Xspikes spikeData sumspiketimeSw skipped] = xSpikesEEmclus(index_beg_spike,data,Chn_ind,Ts,N_chn,maxpeakamp,maxpeakwidth,neighbors_ind,nIntracellsignals,std)
...
%Window of interest must be 32samples (for mclust
WOI = double(([-8/Ts -8/Ts; 23/Ts 23/Ts])); 
N_chn = double(N_chn); Ts = double(Ts);
...
%     Xspikes = zeros((int32(WOI(1)*Ts)+int32(WOI(2)*Ts)+1)*(n_neighbors+1),size(index_beg_spike,1),'int16');
    Xspikes = zeros(32*(n_neighbors+1),size(index_beg_spike,1),'int16');
else
    Xspikes = zeros(32*(n_neighbors+1),size(index_beg_spike,1),'single');
end
...
if ~isempty(index_beg_spike)
    ...
    for (ii  = 1:size(index_beg_spike,1)) % for all spikes found
        ...
        if ((temp == sweepnum & temp2 == sweepnum)) %check window around index_beg_spike exists in the same sweep
            ...
            temp1 = double(data (int32(electrode_index_beg_spike-PreWindow*Ts):int32(temp2)))';
            ...
            if( Peak1 - lastPeak1 )> Ts*1e-3   % must be at least 1ms since last spike
                ...
                if (timeInsweep > 0 ) %% (what is this condition?
                    ...
                    try  %for DEBUGGING easily
                        ...
                        temp1 = data(int32((Peak1(1,1)-int32(8))):int32(Peak1(1,1)+int32(23)));
                        %% UNTESTED
                        for n  = 1: n_neighbors
                            temp = data(int32(double(neighbor_index_peak_spike_index(n))-int32(8)):int32(double(neighbor_index_peak_spike_index(n))+int32(23)));
                            temp1 = [temp1 temp];
                        end
