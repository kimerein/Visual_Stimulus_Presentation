...
try
    if isempty(dir(handles.USER.logdirpath));     parentfolder(handles.USER.logdirpath,1);end
    if isempty(dir(handles.USER.paramdirpath));     parentfolder(handles.USER.paramdirpath,1);end
    if isempty(dir(handles.USER.logdirpath));     parentfolder(handles.USER.logdirpath,1);end
    s = sprintf('ping -n 1 %s', handles.USER.DAQPCIP);
    [junk r] = dos(s);
    if ~isempty(regexp(r,'Request timed out.'))
        error([s 'cannot contact DAQ computer'])
    else
        printf('Successful ping of DAQPCIP: %s, handles.USER.DAQPCIP');
    end
catch ME % debugging
     getReport(ME)
    keyboard
...
S = Screen('Screens'); % take care of case where there is only 1 screen
if S==0; set(handles.ScreenNum,'String','0'); end
