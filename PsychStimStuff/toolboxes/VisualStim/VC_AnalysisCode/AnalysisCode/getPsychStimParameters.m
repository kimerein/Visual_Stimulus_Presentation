...
if ~bdebugParam & ~isempty(dir([ftemp '.*']))
    load(ftemp);
    fnameParams = fullfile(LOGFILEPATH,params.sfilename);
else
...
if exist('maskstr','var') % backward compatibilyt with files that doesn't have this param 
...
sdata = getFileTime(fullfile(DAQFILEPATH,[sfilename '.daq']));
% new version of
...
printf('\n*******************************\n\t%s\t%s',sfilename,datestr(sdata.Creation));
...
if temp2(ind(1),5)> 0.5 % warning if daq and param files differ by more than a half min
    printf('Watch out POSSIBLE mismatch between DAQ and PychStimControl parameters files because are far apart in time\n Can Try 2nd closest Param file')
end
ui = input(stemp);
