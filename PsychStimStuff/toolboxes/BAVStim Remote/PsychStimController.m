...
PSCrigSpecific; % TO DO add case where rigSpecific doesn't exist
...
if length(screennums) == 1; % Check if more than 1 monitor exists
    ...
    % KR hacked fix
    %set(handles.ScreenNum,'String',num2str(1))
end
...
%         black=0; 
%         grey = round(0.5*(black+white));
%         % Set white, black, gray pixel values
%         whitePixVal = lum2pixVal(white);
%         blackPixVal = lum2pixVal(black);
%         grayPixVal = lum2pixVal(grey);
%         white=whitePixVal;
%         black=blackPixVal;
%         grey=grayPixVal;
        ...
        if stim == 1 || stim == 2 || stim == 4 || stim == 5 || stim == 6 || stim == 7
            ...
            % KR - Change background screen color
            offclut(:,:)=round((white-black)*(str2num(get(handles.BkgrndColor,'String'))/100)+black);
        else %% stim == 3
            clut = 0; % movie
        end
...
%             Ncond_pertrial
%             condList
            frameChangeBox(window,windowRect,255); % make sure frame box always ends a stimulus low adds box to corner of screen and switch box from gray to white everytime function is called

            % KR - Change background screen color
            offclut(:,:)=round((white-black)*(str2num(get(handles.BkgrndColor,'String'))/100)+black);
            %moglClutBlit(window,textures(1),offclut);
            % KR 7/16/10 - Not sure if the following flip is necessary, might be able to
            % remove in future version, if don't use moglClutBlit before
            % this flip, this flip will flash gray screen
            %vbl = Screen('Flip',window);
            %vbl=Screen('Flip',window,0,2);
            vbl=0;
...
% handles.UCparams.c
            ...
            if ~clut
                ...
                if clearBkgrnd
                    % KR - Change background screen color
                    Screen('FillRect',window,round((white-black)*(str2num(get(handles.BkgrndColor,'String'))/100)+black));
            ...
            elseif clut
                ...
                if exist('u','var');
                    if isequal(u.status,'open')
                        fwrite(u,[fname '*' condNAME]);
                        sendParams_udp(u,handles);
                        disp('Stim conditions are being transmitted to DAQ PC!');
                    end
                else
                    display(' ********** WARNING Stim conditions are not being transmitted to DAQ PC******');
...
%                     img = 1+sign(sin(f*x).*sin(f*y));
                    disp(handles.phase(c));
                    img = 1+sign(sin(f*x+(handles.phase(c)*2*pi/360)).*sin(f*y));
                    if contrast>1
                        contrast=1;
                    end
            ...
            case 4 %
                ...
                % KR - Change wait period screen color
                offclut(:,:)=round((white-black)*(str2num(get(handles.BkgrndColor,'String'))/100)+black);
                textures = zeros(params.nCond,1);
                for c = 1:params.nCond
...
function BkgrndColor_Callback(hObject, eventdata, handles)
end

function BkgrndColor_CreateFcn(hObject, eventdata, handles)
if ispc
    set(hObject,'BackgroundColor','white');
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
