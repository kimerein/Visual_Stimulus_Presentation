...
% handles.UCparams.c
            ...
            if ~clut
                ...
                % Done with stimulus
                waitScreenColor=3; % white is 1, gray is 2, black is 3
                
                if clearBkgrnd
%                   % KR - Change background screen color 
                    if waitScreenColor==1
                        Screen('FillRect',window,white);
                    elseif waitScreenColor==2
                        Screen('FillRect',window,grey);
                    elseif waitScreenColor==3
...
%                     end
    ...
    function everyframeHelper()
        ...
        putvalue(parentuddobj,[bitOn bitOn],bitLine);
        % KR if baselineYes==1
        %    % KR - add blank screen for baseline measurement
        %    WaitSecs(0.5);
        %end
        if f > 1
            vbl = Screen('Flip',window, vbl + (FrameWait - 0.5) * FrameInt);
    ...
    function switchStimHelper()
        ...
        switch stim
            ...
            case 4 %
                ...
                screenColor=1; % white is 1, gray is 2, black is 3
                if screenColor==1
                    offclut(:,:)=white;
                elseif screenColor==2
                    offclut(:,:)=grey;
                elseif screenColor==3
                    offclut(:,:)=black;
                end
...
elseif  keyCode(UCkeys.strikeKey) % end immediately
    % KR - Print out stimulus location, size and orientation
    disp('UCparams.rx');
    disp(UCparams.rx);
    disp('UCparams.ry');
    disp(UCparams.ry);
    ...
    disp('UCparams.mY');
    disp(UCparams.mY);
    disp('UCparams.rotation');
    disp(UCparams.rotation);  
    disp('Ending Stim. Orientation for GUI');
    disp(360-UCparams.rotation);
