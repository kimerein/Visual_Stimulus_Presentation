...
%
%      PSYCHSTIMCONTROLLER('CALLBACK',hObject,eventData,handles,...) calls
%      the local
...
% Last Modified by GUIDE v2.5 04-Feb-2010 19:58:51
...
else error('rigSpecific file must specify both PSC_REMOTECONTROL_REMOTEPC_PORT and PSC_REMOTECONTROL_REMOTEPC_IP'); end
if exist('PSC_REMOTECONTROL_LOCALPC_PORT','var')
    handles.USER.PSC_REMOTECONTROL_LOCALPC_PORT = PSC_REMOTECONTROL_LOCALPC_PORT;
else  handles.USER.PSC_REMOTECONTROL_LOCALPC_PORT = '3458' ; end

if ~dosping(PSC_REMOTECONTROL_REMOTEPC_IP); display(['Cannot ping REMOTE CONTROL IP: ' PSC_REMOTECONTROL_REMOTEPC_IP]); end % check that can ping ip address
...
handles.orgResolution = Screen('Resolution',str2num(get(handles.ScreenNum,'String'))); % save it so it can be put back to this resolution afte rstimulus is run

% define what the screen resolution will be during stimulus based on
...
function StimType_Callback(hObject, eventdata, handles)
 set([handles.StimType, handles.Orient0,handles.Speed0,handles.Speed0,handles.Freq0,handles.Contrast0,handles.PositionX0,handles.PositionY0,...
            ...
            handles.pushbutton9, handles.LoadParams,  handles.SaveParams,  handles.RunBtn, handles.Var1,handles.Var2, handles.Start1, handles.Start2,...
...
%% put this is a try/catch, so that any crash won't leave screen hung
clc;
...
<<<<<<< .mine
if  get(handles.checkbox8,'Value') && get(handles.toggleMaster,'Value')   % Remote MASTER
...
%         set(handles.figure1,'KeyPressFcn',{@remoteMasterKeyHandler,handles});
    else
...
% clear mex
        ...
        while ~handles.UCparams.doneStim
            ...
            if handles.UCparams.bautoChangeContrast % BA allows manual control of condition
                handles.UCparams.c = condList(mod(iter,params.nCond)+1);
            end
            if sync == handles.USER.Sync.ktdtSync
            ...
            if ~clut
                ...
                if exist('u','var');           if isequal(u.status,'open') fwrite(u,[fname '*' condNAME]); end
                else
            ...
            elseif clut
                ...
                for f = 1:nFrames
                    ...
                    Screen('DrawText',window,sprintf('D:%d %d %dd %1.2fcpd %1.1fhz %1.2f C%d %d %d',ndropframes, vbli, mod(handles.orient(handles.UCparams.c)+(360-handles.UCparams.rotation),360),handles.freq(handles.UCparams.c),handles.TempFreq(handles.UCparams.c),handles.contrast(handles.UCparams.c),params.nCond,iter,stimulusrep),10,2,black);
                    everyframeHelper();
    ...
    function cleanupHelper
        ...
        condListStore = condListStore((condListStore>0));       vblstore = vblstore(~isnan(vblstore));
       ...
       SetResolution(str2num(get(handles.ScreenNum,'String')),handles.orgResolution);
        ...
        if ~get(handles.toggleMaster,'Value'); udpcommhelper(handles.uR,'stimend')  ;      end
    ...
    function everyframeHelper()
        ...
        vbli = vbli+1;
        vblstore(vbli)= vbl;
        if (vblstore(vbli)-vblstore(vbli-1))> FrameInt*1.5;   ndropframes = ndropframes  +1 ; end % define dropped frame
...
if isfield(handles,'eyeCond0'); params.eyeCond0 = str2double(get(handles.eyeCond0,'String')); else params.eyeCond0 = 0; end; % removed eye (left this for backward compatibility)
...
%        str2double(get(hObject,'String')) returns contents of PixelsX as a
%        double
end
% --- Executes during object creation, after setting all properties.
...
% hObject    handle to remoteIP (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% TO DO (currently this fucntion does nothing
...
handles =  guidata(handles.figure1); % must update old version of handles may be pasted in
S = fscanf(obj);
...
if get(handles.checkbox8,'Value') % remote control ond
    ...
    set([handles.text23,handles.ScreenNum, handles.PixelsX, handles.PixelsY, handles.text16, handles.text17,  handles.SizeY, handles.SizeX, handles.text17, handles.ScreenDist , handles.text15, handles.FrameHz, handles.text24]...
        ,'Enable','on');
    set([handles.LocalIP, handles.remoteIP, handles.pushbutton9,handles.toggleMaster],'Enable','off');
    toggleenable('on')
...
% hObject    handle to localip (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of localip as text
%        str2double(get(hObject,'String')) returns contents of localip as a double
end
...
% Hints: contents = get(hObject,'String') returns popmenuMask contents as
% cell array
